...
const reproQuestions = {
	repro_quality: score({
		question: "How completely does the report let a maintainer reproduce the problem without guessing?",
		focus: "Judge only what is written in `issue.sections` (usually `reproduction` together with `actual`). Links to documentation, other issues, screenshots, or CI logs are evidence but are not steps."
	}, [
...
const checks = [
	defineCheck({
		...
		decide(answers, ctx) {
			...
			if (!quality) return {
				status: "abstained",
				note: "no answer for repro_quality",
				gate: "unsure"
			};
			const evidence = { steps: `${quality.score.toFixed(1)}/${quality.top}` };
			...
			if (quality.score >= t.reproOk && quality.confidence >= t.reproConfidence) return {
				status: "decided",
				add: [],
			...
			if (quality.score <= t.reproLow && quality.confidence >= t.reproConfidence) return {
				status: "decided",
